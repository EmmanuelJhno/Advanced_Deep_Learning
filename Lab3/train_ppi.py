...
class MultiHeadGAT(nn.Module):
    ...
    def __init__(self, in_feats, out_feats, num_heads, residual, merge='avg', **kwargs):
        super().__init__()
        self.gat_conv = GATConv(in_feats=in_feats, out_feats=out_feats, residual=residual,
                                num_heads=num_heads, **kwargs)
        self.merge = merge

    def forward(self, graph, inputs):
        outputs = self.gat_conv(graph, inputs)
        if self.merge == 'avg':
...
class CustomGraphModel(nn.Module):
    ...
    def __init__(self, g, n_layers, input_size, hidden_size, output_size, n_heads):
        super().__init__()
        self.g = g
        self.layers = nn.ModuleList()
        self.layers.append(MultiHeadGAT(in_feats=input_size, out_feats=hidden_size,
...
def main(args):
    ...
    model = CustomGraphModel(g=train_dataset.graph, n_layers=4, input_size=n_features,
                             hidden_size=256, output_size=n_classes, n_heads=8).to(device)
